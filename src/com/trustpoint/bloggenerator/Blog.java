...
public class Blog extends JFrame {
  ...
  private void initBlogFrame() {
    ...
    Dimension inputDimension =
        new Dimension((screenSize.width) / 2 - Value.FLOWLAYOUT_GAP * 3 - labelDimension.width,
    ...
    Dimension categoryListDimension =
        new Dimension((screenSize.width) / 2 - Value.FLOWLAYOUT_GAP * 5 - labelDimension.width
            - inputDimensionHalf.width - buttonDimension.width, inputDimension.height);
    ...
    abbrRecordShortDimension =
        new Dimension(buttonDimension.width, addAbbrInputShort.getPreferredSize().height);
    abbrRecordFullDimension =
        new Dimension((screenSize.width) / 2 - Value.FLOWLAYOUT_GAP * 4 - buttonDimension.width * 2,
  ...
  private void initAbbrPanel() {
    ...
    for (Map.Entry<String, String> abbrRecord : abbr.getList().entrySet()) {
      ...
      int emptyWidth = blogFrame.getSize().width / 2 - abbrRecordShortDimension.width
          - abbrRecordFullDimension.width - buttonDimension.width - Value.FLOWLAYOUT_GAP * 4;

      // Fill the empty space on the right of each line
      if (emptyWidth > 0) {
        JLabel emptyLabel = new JLabel();
        emptyLabel.setPreferredSize(
            new Dimension(emptyWidth - Value.FLOWLAYOUT_GAP, emptyLabel.getPreferredSize().height));
    ...
    JLabel emptyLabel = new JLabel(" ");
    emptyLabel.setPreferredSize(new Dimension(blogFrame.getSize().width / 2 - Value.FLOWLAYOUT_GAP,
      ...
      if (paragraphs.get(i).runs.size() > 1) {
        ...
        while (runIndex < paragraphs.get(i).runs.size()) {
          if (paragraphs.get(i).runs.get(runIndex - 1).closeTag.equals("")
            ...
            paragraphs.get(i).runs.get(runIndex - 1).text +=
                paragraphs.get(i).runs.get(runIndex).text;
            paragraphs.get(i).runs.get(runIndex - 1).closeTag =
                paragraphs.get(i).runs.get(runIndex).closeTag;
   ...
   *
   * @param run The run to set style.
   */
   ...
   *
   * @param lastRun Last run instance.
   * @param currentRun Current run instance.
   */
   ...
   *
   * @param run The run instance to close tag on.
   */
      ...
      while (runIndex < paragraphs.get(i).runs.size()) {
        ...
        // Else add abbr tag if text contains short form
          ...
          if (!abbrToAdd.equals("")) {
            ...
            tempRun.closeTag = "";
            newContentRun.text = originalRunText.substring(abbrIndex + abbrToAdd.length());
  ...
  private void getCategoriesInput() {
    ...
    for (int i = 0; i < paragraphs.size(); i++) {
      for (int j = 0; j < paragraphs.get(i).runs.size(); j++) {
        String[] text = StringUtils.split(paragraphs.get(i).runs.get(j).text);
        for (int k = 0; k < text.length; k++) {
          for (Map.Entry<String, Integer> catRecord : CategoryList.categoryCount.entrySet()) {
            if (StringUtils.equalsIgnoreCase(text[k], catRecord.getKey())) {
  ...
  private void trimSpace() {
    for (int i = 0; i < paragraphs.size(); i++) {
      for (int j = 0; j < paragraphs.get(i).runs.size(); j++) {
        ...
        paragraphs.get(i).runs.get(j).text =
            paragraphs.get(i).runs.get(j).text.replaceAll("\\s+", " ");

        // Trim out leading spaces
        if (j == 0 || !paragraphs.get(i).runs.get(j).openTag.equals("")) {
          paragraphs.get(i).runs.get(j).text =
              StringUtils.stripStart(paragraphs.get(i).runs.get(j).text, " ");
        }
        ...
        if (j == paragraphs.get(i).runs.size() - 1
          ...
          paragraphs.get(i).runs.get(j).text =
              StringUtils.stripEnd(paragraphs.get(i).runs.get(j).text, " ");
        }
      }
    }
  }
  ...
  /**
   * Capitalize appropriate words in subtitles.
   */
  private void capitalizeHeader() {
    for (int i = 0; i < paragraphs.size(); i++) {
      if (paragraphs.get(i).openTag.equals(Value.HTML_HEADER_OPEN)) {
        for (int j = 0; j < paragraphs.get(i).runs.size(); j++) {
          String text = paragraphs.get(i).runs.get(j).text;
          paragraphs.get(i).runs.get(j).text = LowercaseWordList.capitalize(text);
        }
      }
    }
  }
  ...
  /**
   * Add contact link in the last paragraph.
   */
  private void addContactLink() {
    int lastParagraphIndex = paragraphs.size() - 1;
    for (int i = 0; i < paragraphs.get(lastParagraphIndex).runs.size(); i++) {
      for (int j = 0; j < Value.CONTACT_KEYS.length; j++) {
        int pos = StringUtils.indexOfIgnoreCase(paragraphs.get(lastParagraphIndex).runs.get(i).text,
            Value.CONTACT_KEYS[j]);
        if (pos != -1) {
          Run tempRun = paragraphs.get(lastParagraphIndex).runs.get(i);
          String originalRunText = tempRun.text;

          tempRun.text = originalRunText.substring(0, pos);

          Run newContentRun = new Run();
          newContentRun.closeTag = tempRun.closeTag;
          tempRun.closeTag = "";
          newContentRun.text = originalRunText.substring(pos + Value.CONTACT_KEYS[j].length());

          Run linkRun = new Run();
          linkRun.openTag = Value.HTML_LINK_OPEN(Value.CONTACT_URL);
          linkRun.text = originalRunText.substring(pos, pos + Value.CONTACT_KEYS[j].length());
          linkRun.closeTag = Value.HTML_LINK_CLOSE;

          paragraphs.get(lastParagraphIndex).runs.add(i + 1, linkRun);
          paragraphs.get(lastParagraphIndex).runs.add(i + 2, newContentRun);
  ...
  private void displayBlog() {
    ...
    List<String> excerptLines =
        displayParagraph(Value.LINE_PREFIX, Value.LINE_LENGTH, paragraphs.get(excerptIndex));
  ...
  /**
   * Format the content of a paragraph to a list of strings to display. Break line if length longer
   * than Value.LINE_LENGTH. NB: Cannot add or leave out spaces when breaking lines.
   *
   * @param prefix The prefix of each line.
   * @param lineLength Line length of each line.
  ...
  private List<String> displayParagraph(String prefix, int lineLength, Paragraph paragraph) {
    ...
    for (int i = 0; i < paragraph.runs.size(); i++) {
      ...
      if (!tempRun.openTag.equals("")) {
        if ((line.length() + tempRun.openTag.length()) > contentLength && breakPoint != -1) {
      ...
      if (!StringUtils.isBlank(tempRun.text)) {
        ...
        for (int k = 0; k < wordList.length; k++) {
          ...
          if ((line.length() + wordList[k].length()) > contentLength && breakPoint != -1) {
            String lineToAdd = line.substring(0, breakPoint);
            String lineRemain = line.substring(breakPoint).trim();
            result.add(prefix + lineToAdd);
      ...
      if (!tempRun.closeTag.equals("")) {
        if ((line.length() + tempRun.closeTag.length()) > contentLength && breakPoint != -1) {
          String lineToAdd = line.substring(0, breakPoint);
          String lineRemain = line.substring(breakPoint).trim();
          result.add(prefix + lineToAdd);
   ...
   *
   * @return true If all inputs are set; false is any input is not set.
   */
   ...
   * @return A string representing the alt of the image.
   */
  private String getImgAlt(String imgName) {
   ...
   *
   * @param shortForm The short form of abbreviation to delete.
   */
  ...
  private void uploadImg() {
    ...
    if (Files.exists(targetDir) && Files.isDirectory(targetDir)) {
      ...
      FileNameExtensionFilter filter =
          new FileNameExtensionFilter("Image only", "jpg", "png", "gif");
  ...
  private void addImgBlog() {
    ...
    if (Files.exists(targetDir) && Files.isDirectory(targetDir)) {
      ...
      if (returnValue == JFileChooser.APPROVE_OPTION) {
        ...
        String imgAlt = getImgAlt(selectedFile.getName());
  ...
  private void addImgExcerpt() {
    ...
    if (Files.exists(targetDir) && Files.isDirectory(targetDir)) {
      ...
      if (returnValue == JFileChooser.APPROVE_OPTION) {
        ...
        excerptImgAlt = getImgAlt(selectedFile.getName());
        displayBlog();
      }
    } else {
      Error error = new Error();
